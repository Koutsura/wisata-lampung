...
<body>
    ...
    <div class="form-container">
        <form action="{{ route('marina.update', $marina->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="nama">Nama Pemesan:</label>
                <input type="text" id="nama" name="nama" value="{{ $marina->nama }}" maxlength="500">
            </div>

            <div class="form-group">
                <label for="nmr_tlp">Nomor HP/Telp:</label>
                <input type="text" id="nmr_tlp" name="nmr_tlp" value="{{ $marina->nmr_tlp }}" maxlength="100">
            </div>

            <div class="form-group">
                <label for="tgl">Tanggal Pesan:</label>
                <input type="date" id="tgl" name="tgl" value="{{ $marina->tgl }}">
            </div>

            <div class="form-group">
                <label for="jml_psr">Jumlah Peserta:</label>
                <input type="number" id="jml_psr" name="jml_psr" value="{{ $marina->jml_psr }}" maxlength="100">
            </div>

            <div class="form-group">
                <label for="jml_hr">Waktu Pelaksanaan Perjalanan:</label>
                <input type="text" id="jml_hr" name="jml_hr" value="{{ $marina->jml_hr }}" maxlength="100">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="hrg_pkt" name="hrg_pkt" "{{ $marina->hrg_pkt }}" readonly>
            </div>

            <div class="form-group">
                <label for="ttl_tgh">Jumlah Tagihan:</label>
                <input type="text" id="ttl_tgh" name="ttl_tgh" "{{ $marina->jml_hr }}" readonly>
            </div>
