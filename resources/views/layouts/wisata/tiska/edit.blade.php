...
<body>
    ...
    <div class="form-container">
        <form action="{{ route('tiska.update', $tiska->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="nama">Nama Pemesan:</label>
                <input type="text" id="nama" name="nama" value="{{ $tiska->nama }}" maxlength="500">
            </div>

            <div class="form-group">
                <label for="nmr_tlp">Nomor HP/Telp:</label>
                <input type="text" id="nmr_tlp" name="nmr_tlp" value="{{ $tiska->nmr_tlp }}" maxlength="100">
            </div>

            <div class="form-group">
                <label for="tgl">Tanggal Pesan:</label>
                <input type="date" id="tgl" name="tgl" value="{{ $tiska->tgl }}">
            </div>

            <div class="form-group">
                <label for="jml_psr">Jumlah Peserta:</label>
                <input type="number" id="jml_psr" name="jml_psr" value="{{ $tiska->jml_psr }}" maxlength="100">
            </div>

            <div class="form-group">
                <label for="jml_hr">Waktu Pelaksanaan Perjalanan:</label>
                <input type="text" id="jml_hr" name="jml_hr" value="{{ $tiska->jml_hr }}" maxlength="100">
            </div>
            ...
            <div class="form-group">
                ...
                <div>
                    <input type="checkbox" id="akomodasi" name="services[]" value="Penginapan"
                        {{ $tiska->akomodasi == 'Y' ? 'checked' : '' }}>
                    <label for="akomodasi">Penginapan (Rp 40.000)</label>
                </div>
                <div>
                    <input type="checkbox" id="transport" name="services[]" value="Transportasi"
                        {{ $tiska->transport == 'Y' ? 'checked' : '' }}>
                    <label for="transport">Transportasi (Rp 250.000)</label>
                </div>
                <div>
                    <input type="checkbox" id="service" name="services[]" value="Servis/Makan"
                        {{ $tiska->service == 'Y' ? 'checked' : '' }}>
                    <label for="service">Servis/Makan (Rp 70.000)</label>
                </div>
            </div>

            <div class="form-group">
                <label for="hrg_pkt">Harga Paket Perjalanan:</label>
                <input type="text" id="hrg_pkt" name="hrg_pkt" "{{ $tiska->hrg_pkt }}" readonly>
            </div>

            <div class="form-group">
                <label for="ttl_tgh">Jumlah Tagihan:</label>
                <input type="text" id="ttl_tgh" name="ttl_tgh" "{{ $tiska->jml_hr }}" readonly>
            </div>
